...
function generateBreastReportText(data) {
    let text = "";
    const addPara = (content) => {
        if (content && content.trim() !== "") text += `<p>${content}</p>`;
    };
    ...
    addPara(`<strong>CLINICAL INFORMATION:</strong> ${data.age || '—'} year old ${String(data.sex || 'patient').toLowerCase()}. Indication: ${data.primaryIndication || '—'}.`);

    let tech = `<strong>TECHNIQUE:</strong> ${data.modalityEvaluated || 'Breast imaging'} performed.`;
    if (data.comparisonAvailable === 'Yes') {
        tech += ` Comparison with prior ${data.priorStudyModality || 'study'}${data.priorStudyDate ? ` dated ${data.priorStudyDate}` : ''}.`;
    }
    addPara(tech);

    text += `<p><strong>FINDINGS:</strong></p>`;

    if (data.modalityEvaluated !== 'Ultrasound Only') {
        addPara(`<strong>Mammography:</strong> Breast density ${data.density || 'not specified'}. Masses: ${data.massPresent || 'No'}. Calcifications: ${data.calcificationsPresent || 'No'}${data.calcificationMorphology ? ` (${data.calcificationMorphology}, ${data.calcificationDistribution || 'distribution not specified'})` : ''}. Architectural distortion: ${data.architecturalDistortion || 'No'}${data.architecturalDescription ? ` (${data.architecturalDescription})` : ''}.`);
        if (Array.isArray(data.masses) && data.masses.length > 0) {
            data.masses.forEach((m, i) => addPara(`Mass ${i + 1}: ${m.side || '—'}, ${m.quadrant || '—'}, shape ${m.shape || '—'}, margin ${m.margin || '—'}, density ${m.density || '—'}.`));
        }
    }

    if (data.modalityEvaluated !== 'Mammogram Only') {
        addPara(`<strong>Right Breast Ultrasound:</strong> Echotexture ${data.rightBackgroundEchotexture || '—'}. Solid lesions: ${data.rightSolidLesions || 'No'}. Cysts: ${data.rightCysts || 'No'}.`);
        addPara(`<strong>Left Breast Ultrasound:</strong> Echotexture ${data.leftBackgroundEchotexture || '—'}. Solid lesions: ${data.leftSolidLesions || 'No'}. Cysts: ${data.leftCysts || 'No'}.`);
    }

    addPara(`<strong>Axillary Nodes:</strong> Significant lymphadenopathy: ${data.lymphadenopathy || 'No'}${data.lymphadenopathy === 'Yes' ? ` (${data.lymphNodeSide || '—'}, fatty hilum ${data.fattyHilum || '—'}, cortical thickness ${data.corticalThickness || '—'} mm)` : ''}.`);

    addPara(`<strong>IMPRESSION:</strong> ${data.impressionSummary || 'No acute abnormality.'}`);
    if (data.recommendation) addPara(`<strong>RECOMMENDATION:</strong> ${data.recommendation}`);
    if (data.biradsCategory) addPara(`<strong>FINAL BI-RADS:</strong> ${data.biradsCategory}`);
    addPara(`<strong>DISCLAIMER:</strong> NOTE: Mammography and other imaging investigations may not reveal 10-15% of cancers. In all cases the clinical features should be taken into account for management decisions such as biopsy and excision, despite the mammogram appearing normal.`);
