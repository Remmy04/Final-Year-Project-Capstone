...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (reportTextBox) {
      if (reportDoc.generatedReport && reportDoc.generatedReport.text) {
          ...
          const templateType = (reportDoc.templateType || '').toLowerCase();
          let generatedHTML = '';
          if (templateType.includes('breast') && typeof generateBreastReportText === 'function') {
              generatedHTML = generateBreastReportText(t);
          } else if (typeof generateLiverReportText === 'function') {
              generatedHTML = generateLiverReportText(t);
          }
  ...
  if(backBtn) {
      backBtn.onclick = () => {
          if(linkedChatId) {
              const templateType = (reportDoc.templateType || '').toLowerCase();
              const backPage = templateType.includes('breast') ? 'Breast_Mammography_AI.html' : 'CT_MR_Liver_AI.html';
              window.location.href = `${backPage}?sessionId=${linkedChatId}`;
          }
